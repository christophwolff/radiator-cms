...
export default {
  create: data => {
    ...
    return apiClient.post(`contributions`, query, {
      headers: {
  ...
  deleteContribution: data => {
    return apiClient.delete(`contributions/${data.contributionId}`, {
      headers: {
  ...
  update: data => {
    ...
    query.append('contribution[contribution_role_id]', data.contributionRoleId)
    query.append('contribution[person_id]', data.personId)
    return apiClient.patch(`contributions/${data.contributionId}`, query, {
      headers: {
        'Content-Type': 'multipart/form-data',
        Authorization: 'Bearer ' + data.token
      }
